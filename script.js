...
(function () {
  ...
  if (parallaxEls.length > 0) {
    window.addEventListener('scroll', () => {
      parallaxEls.forEach(el => {
        ...
        const elementPos = el.getBoundingClientRect().top + scrollY;
        const relativeScroll = scrollY - (elementPos - window.innerHeight);
        const offset = (relativeScroll * 0.5);
  ...
  ============================================================ */
  const smoothScrollElements = document.querySelectorAll('.service-card, .approach-card, .project-card, .value-card, .job-card, .hse-card');
  
  const smoothObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        entry.target.style.animation = 'fade-scale-in 0.6s cubic-bezier(0.4,0,0.2,1) forwards';
        smoothObserver.unobserve(entry.target);
      }
    });
  }, { threshold: 0.1, rootMargin: '0px 0px -50px 0px' });

  smoothScrollElements.forEach(el => smoothObserver.observe(el));

  /* ============================================================
     SMOOTH LINK HOVER EFFECTS
  ============================================================ */
  document.querySelectorAll('.service-link, .nav-link').forEach(link => {
  ...
  /* ============================================================
     MOUSE VELOCITY SCROLL SMOOTHING
  ============================================================ */
  let lastScrollTop = 0;
  let velocityY = 0;
  ...
  window.addEventListener('scroll', () => {
    ...
    velocityY = currentScroll - lastScrollTop;
  ...
  document.querySelectorAll('a').forEach(link => {
    ...
    if (style.position !== 'relative' && !link.parentElement.classList.contains('nav-link')) {
  ...
  window.addEventListener('load', () => {
    ...
    document.body.style.animation = 'fade-scale-in 0.8s cubic-bezier(0.4,0,0.2,1)';
  });
